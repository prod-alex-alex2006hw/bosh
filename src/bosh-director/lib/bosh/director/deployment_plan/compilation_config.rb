  ...
  module DeploymentPlan
    ...
    class CompilationConfig
      ...
      def parse_vm_type_and_vm_requirements(compilation_config, vm_types)
        ...
        vm_requirements = safe_property(compilation_config, 'vm', class: Hash, optional: true)
      ...
      def parse_vm_extensions(compilation_config, vm_extensions)
        ...
        Array(vm_extension_names).each {|vm_extension_name|
          ...
          if @vm_type.nil? && @vm_requirements.nil?
            raise Bosh::Director::CompilationConfigBadVmConfiguration,
              "Compilation config is using vm extension '#{vm_extension.name}' and must configure a vm type or vm block."
          end
