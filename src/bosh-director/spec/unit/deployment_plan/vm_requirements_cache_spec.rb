...
module Bosh::Director::DeploymentPlan
  describe VmRequirementsCache do
    subject {VmRequirementsCache.new(cloud_factory)}
    ...
    it 'returns cloud_properties from the CPI' do
      result = subject.get_vm_cloud_properties('z1', vm_requirements)
    ...
    context 'when two azs use two cpis' do
      context 'when the vm_requirements are the same' do
        it 'multiple calls to the same az return the cached value' do
          first_result = subject.get_vm_cloud_properties('z1', vm_requirements)
          second_result = subject.get_vm_cloud_properties('z1', vm_requirements)
        ...
        it 'multiple calls to the different azs call the cpi' do
          first_result = subject.get_vm_cloud_properties('z1', vm_requirements)
          second_result = subject.get_vm_cloud_properties('z2', vm_requirements)
    ...
    context 'when the vm_requirements are NOT the same' do
      ...
      it 'multiple calls to the same az call the cpi' do
        ...
        first_result = subject.get_vm_cloud_properties('z1', vm_requirements)
        second_result = subject.get_vm_cloud_properties('z1', vm_requirements2)
    ...
    context 'when two azs use the same cpi' do
      let(:cloud_config) do
      ...
      context 'when the vm_requirements are the same' do
        it 'multiple calls returns the cached value' do
          allow(fake_cpi2).to receive(:calculate_vm_cloud_properties)
          ...
          expect(first_result).to eq(second_result)
          expect(fake_cpi1).to have_received(:calculate_vm_cloud_properties).once
          expect(fake_cpi2).to_not have_received(:calculate_vm_cloud_properties)
        end
      end
    end
  end
